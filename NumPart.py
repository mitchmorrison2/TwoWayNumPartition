...
def trivial_implementation(elements):
    ...
    min_diff = 1000000
    # set of sets for all combinations generated by each iteration of loop on line 23
    combos = [[]]

    len_elements = len(list(elements))
    for i in range(len_elements):
        combos.append(itertools.combinations(elements, i))
...
def robust_implementation(elements):
    # performing two way set partition using greedy heuristic algorithm
    sorted_elements = sorted(map(int, elements), reverse= True)
    set1 = []
    set2 = []
    ...
    var = True
    count = 0
    while count < len(sorted_elements):
        ...
        if set_diff > 0:
            ...
            if sorted_elements[count] > sorted_elements[count+1]:
                # add larger element to set2
                set2.append(sorted_elements[count])
                set1.append(sorted_elements[count+1])
                set_diff = set_diff - (sorted_elements[count] - sorted_elements[count+1])
            else:
                set1.append(sorted_elements[count])
                set2.append(sorted_elements[count+1])
                set_diff = set_diff - (sorted_elements[count+1] - sorted_elements[count])
        ...
        # increment counter by 2 because moving 2 elements at a time
        count = count + 2

        # edge case for odd amounts of elements in a set
        if count == len(sorted_elements) - 1:
            print(sorted_elements[count], " ")
            print(sum(set1), sum(set2))
            if set_diff > 0:
                set2.append(sorted_elements[count])
                set_diff = set_diff - sorted_elements[count]
            else:
                set1.append(sorted_elements[count])
                set_diff = set_diff + sorted_elements[count]
            break
